...
export function UiModal({ width = 'md', className, children, isOpen = true, onClose }) {
	...
	return <div
		onClick={handleClick}
		className={clsx(
			"fixed inset-0 bg-slate-900/60 backdrop-blur overflow-y-auto",
			className)}>
		<div
			data-id='modal'
			className={clsx(
...
UiModal.Header = function UiModalHeader({ children, className }) {
	return (
		<div className={clsx("px-6 pt-6 text-2xl", className)}>{children}</div>
	)
}
...
UiModal.Footer = function UiModalHeader({ children, className }) {
	return (
		<div className={clsx("mt-auto p-6 flex gap-4 justify-end", className)}>{children}</div>
